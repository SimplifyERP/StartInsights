...
def mark_booked_status(booking_id,start_time,end_time,date):
    get_book_an_expert = frappe.get_doc("Fundraising Experts", booking_id)
    get_booking_table = frappe.db.get_all("Fundraising Expert Table",filters={'parent': get_book_an_expert.name},fields=['*'],order_by='idx ASC')
    for entry in get_booking_table:
        if entry.date == date:
            if get_time(entry.start_time) == get_time(start_time) and get_time(entry.end_time) == get_time(end_time):
                frappe.db.set_value("Fundraising Expert Table",entry.name,'booked_status','1')
                frappe.db.set_value("Fundraising Expert Table",entry.name,'status','True')
