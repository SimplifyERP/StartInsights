...
def get_fundability_quiz(user_id,customer_group,question_id,option,attempt_status,type_of_question,fundability_quiz_response_id,quiz_completed):
    decode_json_option = json.loads(option)
    fundability = []
    status = False
    message = ""
    option_map = {
    ...
    try:
        ...
        quiz_list = []
        if not (question_id and decode_json_option == []) and type_of_question == "" :
            ...
            if type_of_question == "Single" and len(decode_json_option) == 1:
                ...
                if quiz_flow_table == option_map.get(0):
                    ...
                    next_question = frappe.db.get_value("Fundability Quiz Flow Table",{"parent":get_quiz,"fundability_quiz_question":question_id,"select_option":option_map.get(decode_json_option[0])},["next_display_question_no"])   
    ...
    if type_of_question == "Single" and len(decode_json_option) == 1: 
        ...
        if not frappe.db.exists("Fundability Quiz User Response",{"user":user_id}):
            ...
            response_dict = {
                ...
                "user_response_option": decode_json_option[0],
        ...
        response_list = []
        total_mark = 0
        for options in decode_json_option:
            if options == 1:
        ...
        if not frappe.db.exists("Fundability Quiz User Response", {"user": user_id}):
            ...
            for selected_option in decode_json_option:
